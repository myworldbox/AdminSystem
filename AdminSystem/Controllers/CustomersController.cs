    ...
    public class CustomersController : Controller
    {
        ...
        public IActionResult Index(string search = "", string category = "", string sort = "客戶名稱", string order = "asc")
        {
            ...
            ViewBag.Categories = new SelectList(
                Enum.GetValues(typeof(CustomerCategory)).Cast<CustomerCategory>().Select(e => e.ToString()),
                category);
        ...
        {
            ViewBag.客戶分類 = new SelectList(Enum.GetValues(typeof(CustomerCategory)));
            return View();
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Create([Bind("Id,客戶名稱,統一編號,電話,傳真,地址,Email,客戶分類")] 客戶資料 customer)
        {
        ...
        public IActionResult Edit(int id)
        {
            var customer = _unitOfWork.Customers.GetById(id);
            if (customer == null) return NotFound();
            ViewBag.客戶分類 = new SelectList(Enum.GetValues(typeof(CustomerCategory)), customer.客戶分類);
            return View(customer);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Edit([Bind("Id,客戶名稱,統一編號,電話,傳真,地址,Email,客戶分類")] 客戶資料 customer)
        {
            if (ModelState.IsValid)
            {
